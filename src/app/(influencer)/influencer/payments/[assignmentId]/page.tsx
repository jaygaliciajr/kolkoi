...
export default async function InfluencerAssignmentPaymentsPage({ params }: { params: Params }) {
  ...
  const [campaignRes, milestonesRes, transactionsRes, deliverablesRes] = await Promise.all([
    supabase
      .from("campaigns")
      .select("id, title")
        ...
        {milestones.length === 0 ? (
          ...
          <div className="space-y-2">
            {milestones.map((milestone) => (
              <article key={milestone.id as string} className="rounded-md border border-slate-200 p-3">
                ...
                <div className="mt-2 grid gap-1 text-sm text-slate-700 sm:grid-cols-3">
                  <p>
                    Percent: {typeof milestone.percent === "number" ? `${milestone.percent}%` : "-"}
                  </p>
                  <p>
                    Amount: {toCurrency(typeof milestone.amount === "number" ? milestone.amount : 0)}
                  </p>
                  <p>
